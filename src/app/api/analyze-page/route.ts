...
3. **禁止进行图文一致性检查或事实核查**。不要因为文字内容与图片视觉内容不符而报错（例如：图片是一辆车，文字是“飞机”，这不算错别字）。
4. **仅关注文本本身的错误**。只识别明显的错别字（如“轿车”写成“桥车”）、拼写错误（如“sedan”写成“seden”）和语法错误。
5. **如果修正建议与原文完全一致，请务必返回空数组 []**，绝不要报告“无错误”的错误。
6. **严禁对同义词、近义词或不同的表达习惯进行纠错**。例如“聆听”与“倾听”、“查看”与“查阅”等，只要符合语法且不产生歧义，均视为正确。
7. 仅输出确定的错误。
